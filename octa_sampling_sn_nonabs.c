...
double positive_pcorr (double pcorr) {
    
    // 編相関係数が正ならば+1.0, 負なら1.0を返す
    if (pcorr > 0) {
...
//        for (lp = 0; lp < DIMENSION; lp++) printf ("%lf\n", nuc[5].force[lp]);
        ...
        for (type_lp = 0; type_lp <4; type_lp++) {
            ...
            sprintf (file_name, "../subdata/pote_nm_%s_9.66e-2.txt", type_list[type_lp]);
        ...
        for (type_lp = 0; type_lp < 3; type_lp++) {
            ...
            for (nuc_lp = 0; nuc_lp < 3; nuc_lp++) {
                ...
                pcorr_ratio[type_lp][nuc_lp][0] = positive_pcorr (pcorr[0]);
                pcorr_ratio[type_lp][nuc_lp][1] = positive_pcorr (pcorr[1]);
                pcorr_ratio[type_lp][nuc_lp][2] = positive_pcorr (pcorr[2]);
            }
        }
        fclose (fpr);
...
//        for (lp = 0; lp < DIMENSION; lp++) printf ("%lf\n", spb->force[lp]);
        ...
        for (type_lp = 0; type_lp < 2; type_lp++) {
            ...
            sprintf (file_name, "../subdata/pote_sm_%s_9.66e-2.txt", type_list[type_lp]);
            if ( (fpr = fopen (file_name, "r")) == NULL) {
...
//        for (lp = 0; lp < 6; lp++) printf ("%lf\n", para_list[far][0][lp]);
        ...
        for (mem_lp = 0; mem_lp < 3; mem_lp++) {
            ...
            for (type_lp = 0; type_lp < 2; type_lp++){
                ...
                fscanf (fpr, "%s %lf\n", dummy, &ratio);
                pcorr_ratio[type_lp][mem_lp] = positive_pcorr (ratio);
            }
        }
        fclose (fpr);
    }
...
//    char directory[] = "/Users/tkym/Desktop/Imaris/analysis/06-28decon";
    ...
    if ( option == 'c') {
        ...
        for (nuc_lp = 0; nuc_lp < SIZE; nuc_lp++) {
            ...
            f = - K_EXPERIENCE * k_sn * ratio * ( par[0] * (dist - par[1]) * exp1 + par[3] * (dist - par[4]) *exp2 )
            / ( dist * (exp1 + exp2));
...
//            printf ("%c f = %lf\n", option, f);
        ...
        for (type_lp = 0; type_lp < 2; type_lp++) {
            ...
            sprintf (file_name, "../subdata/pote_sn_%s_9.66e-2.txt", type_list[type_lp]);
            if ( (fpr = fopen (file_name, "r")) == NULL) {
...
//        for (lp = 0; lp < 6; lp++) printf ("%lf\n", para_list[far][0][lp]);
        ...
        for (nuc_lp = 0; nuc_lp < 3; nuc_lp++) {
            ...
            for (type_lp = 0; type_lp < 2; type_lp++){
                ...
                fscanf (fpr, "%s %lf\n", dummy, &ratio);
                pcorr_ratio[type_lp][nuc_lp] = positive_pcorr (ratio);
            }
        }
        fclose (fpr);
...
//        printf ("\t Calculating now ...  step = %d\r", step);
        ...
        for ( mitigation = 0; mitigation < WRITE_INTERVAL; mitigation++) {
            ...
            Calculation (mitigation, nuc, spb, k_sn);
        }
